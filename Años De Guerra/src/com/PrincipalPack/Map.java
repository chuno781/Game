...
public class Map {
	...
	private ArrayList<Enemy> enemysDeads=new ArrayList<Enemy>();
        ...
        for(final TMXObjectGroup group: this.mTMXTiledMap.getTMXObjectGroups()) 
        {
        	...
        	if(group.getTMXObjectGroupProperties().containsTMXProperty("enemy", "true"))
	        {
	            for(TMXObject object : group.getTMXObjects()) 
	            {
	        		...
	        		anciano.setStatistics(Integer.parseInt(prop.get(0).getValue())+1000,Integer.parseInt(prop.get(1).getValue()),Integer.parseInt(prop.get(2).getValue()));
	           		mMapScene.attachChild(anciano.getAnimatedSprite());
	           		anciano.addToPhysicsWorld(mPhysicsWorld, app.CHARACTER_FIXTURE_DEF);
	           		anciano.startUpdate();
	...
	public void cargarCharacters() {

            for(int i=0;enemysDeads.size()>i ;) 
            {
            	Enemy e=enemysDeads.get(i);
            	e.reiniciar();
           		enemysDeads.remove(i);
	        }	
    }

	public void addEnemy(Enemy enemy) {
		enemysDeads.add(enemy);
	}
}
